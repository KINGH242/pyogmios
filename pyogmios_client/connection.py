...
from pyogmios_client.server_health import get_server_health, ConnectionConfig, Connection
...
async def create_interaction_context(
    ...
    health = await get_server_health(Options(connection=connection))  # You need to implement this function

    def close_on_completion():
        return (options.interaction_type or 'LongRunning') == 'OneTime'
    
    def executor(resolve: Callable[[Any], None], reject: Callable[[Exception], None]) -> None:
        ...
        def on_open(ws_app: WebSocketApp):
            return resolve(InteractionContext(
                connection=connection,
                socket=ws_app,
